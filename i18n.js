...
function addSelector(){
 ...
 Object.entries(locales).forEach(([code])=>{const o=document.createElement('option');o.value=code;o.textContent=selectorLabels[code]||locales[code]?.label||code;select.append(o)});
 ...
 function normalizeLanguageSelectorOptions(select){
  if(!select||!select.options)return;
  Array.from(select.options).forEach(option=>{
   const code=String(option.value||'').trim();
   const label=selectorLabels[code];
 ...
 function syncLanguageSelectors(){
  document.querySelectorAll('[data-nexus-language]').forEach(select=>{
   normalizeLanguageSelectorOptions(select);
