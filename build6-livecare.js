(function(){
 ...
 $('#openPatientTracker')?.addEventListener('click',()=>{$('#patientAccess').hidden=false;$('#patientAccess').scrollIntoView({behavior:'smooth',block:'center'});$('#reference')?.focus()});
 function renderPatientPulse(t){
  if(!t)return;const status=String(t.status||'SUBMITTED').toUpperCase(),assigned=Boolean(t.driverName||t.vehicleUnit),confirmed=!['SUBMITTED','REVIEWING','PENDING_APPROVAL','PENDING_DISPATCH_CONFIRMATION'].includes(status),tripDate=t.date||t.tripDate||'',tripTime=t.time||t.tripTime||'';
  $('#patientNextTime').textContent=tripTime||'Time pending';$('#patientNextDate').textContent=tripDate||'Date pending';$('#patientRideStatus').textContent=t.statusLabel||labels[status]||status.replaceAll('_',' ');$('#patientRideStatusNote').textContent=status==='EN_ROUTE'?'Your driver is traveling toward pickup':status==='ARRIVED'?'Your vehicle has arrived':status==='PATIENT_ON_BOARD'?'Transportation is in progress':'Updates live from dispatch';$('#patientAssignment').textContent=t.vehicleUnit||t.driverName||'Pending';$('#patientAssignmentNote').textContent=t.driverName?`Driver: ${t.driverName}`:'Shown when assigned';$('#patientService').textContent=t.service||'NEMT';$('#patientReadyTitle').textContent=confirmed?'Your transportation is confirmed':'Your request is being coordinated';$('#patientRoute').textContent=`${t.pickup||'Pickup pending'} → ${t.destination||'Destination pending'}`;$('#patientCheckConfirmation').textContent=`${confirmed?'✓':'○'} ${confirmed?'Ride confirmed by dispatch':'Dispatch confirmation pending'}`;$('#patientCheckAssignment').textContent=`${assigned?'✓':'○'} ${assigned?'Driver or vehicle assigned':'Driver assignment pending'}`;$('#patientCheckTiming').textContent=`${tripDate||tripTime?'✓':'○'} ${tripDate||tripTime?`Pickup scheduled ${tripDate} ${tripTime}`:'Pickup timing will update here'}`;$('#patientPulseIntro').textContent='Showing live details for your authorized transportation record.';
 }
