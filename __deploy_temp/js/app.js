(function(){
 ...
 $('#accessBackToMap')?.addEventListener('click',()=>{const gateway=$('#accessTitle')?.closest('.accessGateway');forceShowAccessGateway=false;if(gateway)gateway.hidden=true;window.scrollTo({top:0,behavior:'smooth'});});
 ...
 function fallbackFleet(){
  const defs=[
   ...
   ['NEX-22','REVIEWING','Medical transport',24,'Prince Georgeâ€™s review zone','Dispatch review']
  ];
 ...
 function showVehicle(index){const v=fleetVehicles[index],m=fleetMarkers[index];if(!v||!m||!fleetMap)return;const pos=interpolateRoute(v.route,v.progress);fleetMap.setView(pos,13);fleetMap.popup.innerHTML=`<strong>${esc(v.unit)}</strong><span>${esc(v.service||'Nexus transport')}</span><b>${esc(v.statusLabel)}</b><small>${esc(v.routeLabel||'Operational route')} Â· ${Math.round(v.progress)}%</small><small>${esc(v.eta||'Monitoring')}</small>`;fleetMap.popup.hidden=false;fleetMap.popup.style.left='50%';fleetMap.popup.style.top='38%'}
...
function drawFilteredFleet(){
  ...
  const mapStatus=$('#fleetMapStatus');if(mapStatus)mapStatus.textContent=`${fleetVehicles.length} vehicle${fleetVehicles.length===1?'':'s'} Â· ${serviceText} Â· ${statusText}`;
...
function renderFleet(data,role,mode){
  ...
  updated.textContent=`Updated ${new Date(data.generatedAt||Date.now()).toLocaleTimeString([], {hour:'numeric',minute:'2-digit',second:'2-digit'})}`;mapStatus.textContent=mode==='fallback'?'Moving fleet preview Â· native map':'Live fleet telemetry connected';
  drawFilteredFleet();
 ...
 function facilityRow(trip,history=false){
  ...
  const route=`<span class="facilityRoute"><b>${esc(trip.pickup||'Pickup pending')}</b><i aria-hidden="true">â†’</i><b>${esc(trip.destination||'Destination pending')}</b></span>`;
  if(history)return `<tr><td><strong>${patient}</strong><small>${esc(trip.patientPhone||'')}</small></td><td><b>${esc(trip.reference||'â€”')}</b></td><td>${esc(trip.service||'Medical transportation')}</td><td>${esc(trip.pickup||'â€”')}</td><td>${esc(trip.destination||'â€”')}</td><td>${esc(trip.date||'â€”')}<small>${esc(trip.time||'')}</small></td><td><span class="status ${rideStatusClass(trip.status)}">${esc(trip.statusLabel||trip.status||'Pending')}</span></td></tr>`;
  return `<tr><td><strong>${patient}</strong><small>${esc(trip.patientPhone||'')}</small></td><td><b>${esc(trip.reference||'â€”')}</b><small>${esc(trip.service||'')}</small></td><td>${route}</td><td>${esc(trip.date||'â€”')}<small>${esc(trip.time||'')}</small></td><td><span class="status ${rideStatusClass(trip.status)}">${esc(trip.statusLabel||trip.status||'Pending')}</span></td><td>${esc(trip.vehicleUnit||'Pending')}<small>${esc(trip.driverName||'Unassigned')}</small></td></tr>`;
 }
 ...
 function roleTripRow(trip,role,history=false){
  ...
  const schedule=`${esc(trip.date||'â€”')}<small>${esc(trip.time||'')}</small>`;
  const route=`<span class="facilityRoute"><b>${esc(trip.pickup||'Pickup pending')}</b><i aria-hidden="true">â†’</i><b>${esc(trip.destination||'Destination pending')}</b></span>`;
  if(role==='DRIVER') return `<tr><td><b>${esc(trip.reference||'â€”')}</b><small>${esc(trip.service||'')}</small></td><td><strong>${esc(trip.patientName||'Patient')}</strong><small>${esc(trip.patientPhone||'')}</small></td><td>${route}</td><td>${schedule}</td><td>${status}</td><td>${esc(trip.notes||'No special instructions')}</td></tr>`;
  if(['DISPATCHER','ADMIN'].includes(role)) return `<tr><td><b>${esc(trip.reference||'â€”')}</b><small>${esc(trip.service||'')}</small></td><td><strong>${esc(trip.patientName||'Patient')}</strong><small>${esc(trip.patientPhone||'')}</small></td><td>${route}</td><td>${schedule}</td><td>${status}</td><td>${esc(trip.vehicleUnit||'Unassigned')}<small>${esc(trip.driverName||'Driver pending')}</small></td></tr>`;
  if(role==='EXECUTIVE') return `<tr><td><b>${esc(trip.reference||'â€”')}</b></td><td>${esc(trip.service||'Transportation')}</td><td>${esc(trip.pickup||'â€”')} â†’ ${esc(trip.destination||'â€”')}</td><td>${schedule}</td><td>${status}</td><td>${esc(trip.vehicleUnit||'Pending')}</td></tr>`;
  if(role==='QA') return `<tr><td><b>${esc(trip.reference||'â€”')}</b></td><td>${esc(trip.service||'Transportation')}</td><td>${status}</td><td>${esc(trip.vehicleUnit||'Pending')}</td><td>${esc(trip.driverName||'Unassigned')}</td><td>${esc(trip.notes||'No quality note')}</td></tr>`;
  if(role==='BILLING') return `<tr><td><b>${esc(trip.reference||'â€”')}</b></td><td>${esc(trip.patientName||'Patient')}</td><td>${esc(trip.service||'Transportation')}</td><td>${schedule}</td><td>${status}</td><td>${esc(trip.facilityId||'Direct/private pay')}</td></tr>`;
  return `<tr><td><b>${esc(trip.reference||'â€”')}</b></td><td>${esc(trip.service||'Transportation')}</td><td>${route}</td><td>${schedule}</td><td>${status}</td><td>${esc(trip.vehicleUnit||'Pending')}</td></tr>`;
 }
 ...
 async function loadRideBoard(){
  ...
  $('#fleetMapStatus').textContent='Connecting to fleet telemetryâ€¦';
