 ...
 try{
  ...
  if(p[0]==='admin'&&p[1]==='reset-credentials'&&method==='POST'){
   ...
   return json(200,{ok:true,results:result.results,created:result.created,updated:result.updated,message:`${result.results.length} accounts reset. All credentials restored.`});
    ...
    'dispatcher1@nexusmt.com',
    'dispatcher2@nexusmt.com',
    ...Array.from({length:10},(_,idx)=>`driver${String(idx+1).padStart(2,'0')}@nexusmt.com`)
     ];
     const bookingRefExact=[
    'NMT-DEMO-0001','NMT-DEMO-0002','NMT-DEMO-0003',
    'NMT-DRV-DEMO-1001','NMT-DRV-DEMO-1002','NMT-DRV-DEMO-1003','NMT-DRV-DEMO-1004','NMT-DRV-DEMO-1005','NMT-DRV-DEMO-1006','NMT-DRV-DEMO-1007','NMT-DRV-DEMO-1008','NMT-DRV-DEMO-1009','NMT-DRV-DEMO-1010','NMT-DRV-DEMO-1011','NMT-DRV-DEMO-1012',
    'NMT-DRV-DEMO-2000','NMT-DRV-DEMO-2001','NMT-DRV-DEMO-2002','NMT-DRV-DEMO-2003'
     ];
     const employeeCodes=['DISP001','DISP002',...Array.from({length:10},(_,idx)=>`NEXD${String(idx+1).padStart(3,'0')}`)];
     try{
    await client.query('BEGIN');
    const userRows=await client.query(
     ...
     [userEmails.map((email)=>email.toLowerCase()),['Test %','Demo Driver %']]
    );
    const bookingRows=await client.query(
     `SELECT reference,name,email,notes,status FROM bookings
      WHERE reference=ANY($1::text[])
       OR lower(coalesce(email,'')) LIKE '%@example.com'
       OR name ILIKE ANY($2::text[])
       OR notes ILIKE ANY($3::text[])
       OR notes ILIKE '[WORKLOAD-DEMO]%'
       OR notes ILIKE '%for Fletcher Kalundi%'
       OR notes ILIKE '%transportation request%'`,
     [
      bookingRefExact,
      ['Preview Rider %','Quick Test Rider %','Demo Rider %'],
      ['%Regular dialysis appointment%','%Online booking -%','%Dispatch-scheduled%','%Testing trip within next%']
     ]
    );
    const employeeRows=await client.query(
     `SELECT id,employee_code,display_name,email,role FROM employees
      WHERE employee_code=ANY($1::text[])
       OR lower(email)=ANY($2::text[])
       OR display_name ILIKE ANY($3::text[])`,
     [
      employeeCodes,
      ['dispatcher1@nexusmt.com','dispatcher2@nexusmt.com',...Array.from({length:10},(_,idx)=>`driver${String(idx+1).padStart(2,'0')}@nexusmt.com`)],
      ['Demo Driver %','Dispatcher One','Dispatcher Two']
     ]
    );
    const brokerRows=await client.query(`SELECT id,name,contact_email FROM brokers WHERE lower(contact_email)=ANY($1::text[]) OR name ILIKE ANY($2::text[])`,[['probe@test.com'],['Probe Broker']]);

    const bookingRefs=bookingRows.rows.map((row)=>String(row.reference));
    const userIds=userRows.rows.map((row)=>String(row.id));
    const employeeIds=employeeRows.rows.map((row)=>String(row.id));
    const brokerIds=brokerRows.rows.map((row)=>String(row.id));
    ...
    if(dryRun){
     ...
     return json(200,{
      ok:true,
      dryRun:true,
      counts:{users:userRows.rowCount,bookings:bookingRows.rowCount,employees:employeeRows.rowCount,brokers:brokerRows.rowCount},
      sample:{users:userRows.rows.slice(0,10),bookings:bookingRows.rows.slice(0,10),employees:employeeRows.rows.slice(0,10),brokers:brokerRows.rows.slice(0,10)},
      message:'Dry run complete. Send {"dryRun":false} to delete the matched records.'
     });
    }

    const deleted={users:0,sessions:0,tripWorkAssignments:0,tripStatusHistory:0,bookings:0,employeeShifts:0,employees:0,brokerRates:0,brokers:0};
    if(bookingRefs.length){
     deleted.tripWorkAssignments=(await client.query('DELETE FROM trip_work_assignments WHERE booking_reference=ANY($1::text[])',[bookingRefs])).rowCount;
     deleted.tripStatusHistory=(await client.query('DELETE FROM trip_status_history WHERE booking_reference=ANY($1::text[])',[bookingRefs])).rowCount;
     deleted.bookings=(await client.query('DELETE FROM bookings WHERE reference=ANY($1::text[])',[bookingRefs])).rowCount;
    }
    if(employeeIds.length){
     deleted.employeeShifts=(await client.query('DELETE FROM employee_shifts WHERE employee_id::text=ANY($1::text[])',[employeeIds])).rowCount;
     deleted.employees=(await client.query('DELETE FROM employees WHERE id::text=ANY($1::text[])',[employeeIds])).rowCount;
    }
    if(userIds.length){
     deleted.sessions=(await client.query('DELETE FROM sessions WHERE user_id::text=ANY($1::text[])',[userIds])).rowCount;
     deleted.users=(await client.query('DELETE FROM users WHERE id::text=ANY($1::text[])',[userIds])).rowCount;
    }
    if(brokerIds.length){
     deleted.brokerRates=(await client.query('DELETE FROM broker_rates WHERE broker_id::text=ANY($1::text[])',[brokerIds])).rowCount;
     deleted.brokers=(await client.query('DELETE FROM brokers WHERE id::text=ANY($1::text[])',[brokerIds])).rowCount;
    }

    await audit('SYSTEM','cleanup-test-data','DELETED',{by:me.email,counts:{users:deleted.users,bookings:deleted.bookings,employees:deleted.employees,brokers:deleted.brokers}});
    await client.query('COMMIT');
    return json(200,{ok:true,dryRun:false,deleted,message:'Matched test/demo data removed successfully.'});
     }catch(err){
    await client.query('ROLLBACK');
    throw err;
     }finally{
