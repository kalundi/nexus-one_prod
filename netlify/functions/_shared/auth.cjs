...
async function requireUser(token,roles=[]){
  ...
  let r;
  try{
    r=await query(`SELECT u.*, s.id AS session_id, s.last_activity_at FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_digest=$1 AND s.revoked_at IS NULL AND s.expires_at>now() AND u.active=true`,[digest(token)]);
  }catch(e){
