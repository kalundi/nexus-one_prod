...
async function sendEmail(to,subject,html){
 if(!process.env.SENDGRID_API_KEY||!process.env.SENDGRID_FROM_EMAIL||!to)return {status:'skipped'};
...
function parseEmailBody(body){
 ...
 return result;
} if(dateMatch){result.trip_date=clean(dateMatch[1]);}
 if(timeMatch){result.trip_time=clean(timeMatch[1]);}
 if(rateMatch){result.broker_quoted_rate=Number(rateMatch[1]);}
 for(const line of lines){
  const lower=line.toLowerCase();
  if(!result.pickup && /(pickup|origin|from)/.test(lower)){
   result.pickup=extractValue(line);
