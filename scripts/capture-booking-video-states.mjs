...
await page.evaluate(() => {
  document.querySelector('#fareConfirmAmount').textContent = document.querySelector('#fareSummaryAmount').textContent;
  document.querySelector('#fareConfirmDetails').textContent = 'Route and appointment estimate';
  document.querySelector('#fareConfirmDialog').showModal();
});
await page.waitForTimeout(250);
...
await page.evaluate(() => {
  ...
  outcome.className = 'bookingOutcome show confirmed';
  document.querySelector('#submitBtn').hidden = true;
  const payment = document.querySelector('#paymentSection');
  payment.hidden = false;
  document.querySelector('#paymentSummary').textContent = 'Booking NMT-20260815-4821 is ready for payment. Estimated total: $128.40.';
