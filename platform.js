...
(function(){
 ...
 function arrangeAdaptiveMobileHeader(){const header=document.querySelector('header'),shell=header?.querySelector('.nav,.globalNav'),nav=header?.querySelector('.globalLinks,.links'),host=header?.querySelector('.nexusSharedActions'),account=header?.querySelector('#nexusAccountMount');if(!header||!shell||!nav||!host||header.querySelector('#primary-navigation'))return;if(matchMedia('(max-width:760px)').matches){header.classList.add('nexusAdaptiveMobileHeader');if(account){account.classList.add('nexusMobileMenuAccount');nav.appendChild(account)}}else{header.classList.remove('nexusAdaptiveMobileHeader');if(account){account.classList.remove('nexusMobileMenuAccount');host.appendChild(account)}}}
