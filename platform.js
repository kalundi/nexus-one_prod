...
(function(){
 ...
 function configureSignupTermsGate(dialog){
  ...
  const status=document.createElement('a');status.className='nexusTermsGateStatus';status.href='/legal.html#terms';status.target='_blank';status.rel='noopener';status.textContent='Open Terms & Conditions';consent.closest('label').before(status);
  let reviewTimer=null,termsWindow=null,startedAt=0,reviewComplete=false;
  const resetReview=()=>{clearInterval(reviewTimer);reviewTimer=null;reviewComplete=false;consent.checked=false;consent.disabled=true;submit.disabled=true;status.classList.remove('is-complete');status.textContent='Open Terms & Conditions'};
  const openTerms=event=>{
   ...
   if(!termsWindow){resetReview();form.querySelector('.nexusLoginError').textContent='Please allow pop-ups, then open Terms & Conditions again.';return}
   reviewTimer=setInterval(()=>{
    if(termsWindow.closed){resetReview();return}
    const remaining=Math.max(0,5-Math.floor((Date.now()-startedAt)/1000));
    if(remaining>0)return;
    clearInterval(reviewTimer);reviewTimer=null;reviewComplete=true;consent.disabled=false;status.classList.add('is-complete');status.textContent='Open Terms & Conditions';
   },250);
  };
 ...
 function normalizeNav(){
  const header=document.querySelector('header');if(!header)return false;const nav=header.querySelector('#primary-navigation,.globalLinks,.links');if(!nav)return false;header.querySelectorAll('.navRight .livecareLink,[data-mobile-livecare]').forEach(link=>link.remove());nav.querySelectorAll(':scope>a[href*="booking-app"]').forEach(link=>link.remove());
  if(!nav.querySelector('.nexusHomeGroup')){const retained=[...nav.querySelectorAll(':scope>a')].filter(link=>{const href=link.getAttribute('href')||'';return href&&href!=='/'&&!href.startsWith('#')&&!href.startsWith('/#')&&!href.includes('livecare')}).map(link=>({href:link.getAttribute('href'),label:link.textContent.trim(),current:link.getAttribute('aria-current')}));nav.innerHTML='';const group=document.createElement('details');group.className='nexusHomeGroup';group.innerHTML=`<summary>Home</summary><div class="nexusHomeMenu"><a class="nexusMobileHomeLink" href="/">Home</a>${sectionLinks.map(([label,id])=>`<a href="/#${id}">${label}</a>`).join('')}</div>`;nav.appendChild(group);const live=document.createElement('a');live.href='/livecare.html';live.textContent='LiveCare';nav.appendChild(live);retained.forEach(item=>{if([...nav.querySelectorAll('a')].some(link=>link.getAttribute('href')===item.href))return;const link=document.createElement('a');link.href=item.href;link.textContent=item.label;if(item.current)link.setAttribute('aria-current',item.current);nav.appendChild(link)});group.querySelectorAll('a').forEach(link=>link.addEventListener('click',()=>group.removeAttribute('open')))}
 ...
 function ensureSharedHeaderActions(){const header=document.querySelector('header');if(!header)return;let host=header.querySelector('.navRight,.globalActions');const shell=header.querySelector('.nav,.globalNav');if(!host){if(!shell)return;host=document.createElement('div');host.className='globalActions nexusSharedActions';shell.appendChild(host)}host.classList.add('nexusSharedActions');let language=host.querySelector('.nexusLanguageControl,.headerLanguage');if(!language){language=document.createElement('label');language.className='nexusLanguageControl headerLanguage nexusCompactLanguage';language.innerHTML='<span class="sr">Language</span><select data-nexus-language aria-label="Language"><option value="en-US">English</option><option value="es">Español</option><option value="fr">Français</option></select>';host.appendChild(language)}let call=host.querySelector('.call,.globalPhone,[href^="tel:+18886395766"]');if(!call){call=document.createElement('a');call.href='tel:+18886395766';call.innerHTML='<span aria-hidden="true">☎</span><span><small>Call Nexus</small><b>(888) 639-5766</b></span>';host.appendChild(call)}call.classList.add('call','nexusSharedCall');let book=host.querySelector('.navBookRide,.navCta,[data-book-ride]');if(!book){book=document.createElement('a');book.href='/booking-app.html';book.textContent='BOOK A RIDE →';host.appendChild(book)}book.classList.add('navBookRide','bookRideSign','nexusSharedBook');const account=host.querySelector('#nexusAccountMount')||header.querySelector('#nexusAccountMount');[language,call,book,account].filter(Boolean).forEach(node=>host.appendChild(node));let menu=header.querySelector('.menuBtn,.mobileNavToggle');const nav=header.querySelector('#primary-navigation,.globalLinks,.links');if(!menu&&shell&&nav){if(!nav.id)nav.id='nexusPrimaryNavigation';menu=document.createElement('button');menu.className='mobileNavToggle';menu.type='button';menu.setAttribute('aria-expanded','false');menu.setAttribute('aria-controls',nav.id);menu.setAttribute('aria-label','Open navigation');menu.innerHTML='<span></span><span></span><span></span>'}if(menu&&shell){shell.appendChild(menu);if(nav&&menu.dataset.nexusUnifiedMenuBound!=='1'){menu.dataset.nexusUnifiedMenuBound='1';menu.addEventListener('click',event=>{event.stopImmediatePropagation();const open=!nav.classList.contains('open');nav.classList.toggle('open',open);menu.setAttribute('aria-expanded',String(open));menu.setAttribute('aria-label',open?'Close navigation':'Open navigation')},{capture:true});nav.addEventListener('click',event=>{if(event.target.closest('a')&&innerWidth<=760){nav.classList.remove('open');menu.setAttribute('aria-expanded','false');menu.setAttribute('aria-label','Open navigation')}})}}}
 function arrangeAdaptiveMobileHeader(){const header=document.querySelector('header'),shell=header?.querySelector('.nav,.globalNav'),nav=header?.querySelector('#primary-navigation,.globalLinks,.links'),host=header?.querySelector('.nexusSharedActions'),account=header?.querySelector('#nexusAccountMount'),homeGroup=header?.querySelector('.nexusHomeGroup');if(!header||!shell||!nav||!host)return;if(matchMedia('(max-width:760px)').matches){header.classList.add('nexusAdaptiveMobileHeader');homeGroup?.setAttribute('open','');if(account){account.classList.remove('nexusMobileMenuAccount');account.classList.add('nexusMobileHeaderAccount');host.appendChild(account)}}else{header.classList.remove('nexusAdaptiveMobileHeader');homeGroup?.removeAttribute('open');if(account){account.classList.remove('nexusMobileMenuAccount','nexusMobileHeaderAccount');host.appendChild(account)}}}
 ...
 let tries=0;function sync(){if(document.getElementById('bookingForm')||/\/booking-app(?:\.html)?$/i.test(location.pathname))return;lockHomeMenuStyles();normalizeNav();ensureSharedHeaderActions();arrangeAdaptiveMobileHeader();addSectionReturns();ensureCarouselRotation();if(tries++<50)setTimeout(sync,120)}window.addEventListener('resize',arrangeAdaptiveMobileHeader,{passive:true});if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',sync,{once:true});else sync();
})();
...
(function(){
	...
	function ensureStyle(){
		...
		style.id=STYLE_ID;
		style.textContent=`
.footer.footerUnified{position:relative;z-index:2;margin-top:24px;padding:0 !important;background:linear-gradient(180deg,#082f49 0%,#07283f 100%);color:#d4e5f1;border-top:1px solid rgba(255,255,255,.12);}
...
.footer.footerUnified .footerSocial a[aria-label="YouTube"] svg,.footer.footerUnified .footerSocial a[aria-label="TikTok"] svg,.footer.footerUnified .footerSocial a[aria-label="Instagram"] svg,.footer.footerUnified .footerSocial a[aria-label="Facebook"] svg,.footer.footerUnified .footerSocial a[aria-label="X"] svg,.footer.footerUnified .footerSocial a[aria-label="Bluesky"] svg{fill:currentColor;stroke:none;}main>section.section{padding-top:0!important;padding-bottom:0!important;margin-top:0!important;margin-bottom:0!important;}main>section.section+section.section{margin-top:0!important;}
...
@media (max-width:620px){.footer.footerUnified .footerAccordionSection{padding:0 !important;overflow:hidden;}.footer.footerUnified .footerAccordionToggle{display:flex !important;align-items:center;justify-content:space-between;width:100%;min-height:56px;padding:12px 14px;border:0;background:transparent;cursor:pointer;text-align:left;}.footer.footerUnified .footerAccordionToggle strong{margin:0 !important;font-size:13px !important;}.footer.footerUnified .footerAccordionIcon{display:block;position:relative;width:24px;height:24px;flex:0 0 24px;border-radius:999px;background:rgba(255,255,255,.1);}.footer.footerUnified .footerAccordionIcon::before,.footer.footerUnified .footerAccordionIcon::after{content:"";position:absolute;left:7px;right:7px;top:11px;height:2px;border-radius:2px;background:#fff;transition:transform .18s ease;}.footer.footerUnified .footerAccordionIcon::after{transform:rotate(90deg);}.footer.footerUnified .footerAccordionToggle[aria-expanded="true"] .footerAccordionIcon::after{transform:rotate(0deg);}.footer.footerUnified .footerAccordionPanel{padding:0 14px 14px;}.footer.footerUnified .footerAccordionPanel[hidden]{display:none !important;}}
	...
	function configureFooterAccordion(grid){
		if(grid.dataset.mobileAccordionApplied==='1')return;
		grid.dataset.mobileAccordionApplied='1';
	...
	function normalizeFooter(){
		...
		if(grid.dataset.unifiedFooterApplied==='1'&&grid.querySelector('.footerBrandCompact')&&grid.querySelector('.footerSocial a[aria-label="X"]')){
			configureFooterAccordion(grid);
			return true;
		}
		delete grid.dataset.mobileAccordionApplied;
		...
		grid.innerHTML=''
			...
			+     '<a href="https://x.com/nexus_m_t" target="_blank" rel="noopener noreferrer" aria-label="X"><svg viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24h-6.657l-5.214-6.817-5.967 6.817H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231 5.45-6.231Zm-1.161 17.52h1.833L7.084 4.126H5.117L17.083 19.77Z"></path></svg></a>'
	...
	function boot(){
		ensureStyle();
		normalizeFooter();
		const observer=new MutationObserver(()=>queueMicrotask(normalizeFooter));
