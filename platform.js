...
(function(){
 ...
 function signupDialog(){let dialog=document.getElementById('nexusSignupDialog');if(dialog)return dialog;dialog=document.createElement('dialog');dialog.id='nexusSignupDialog';dialog.className='nexusLoginDialog';dialog.innerHTML=`<div class="nexusLoginCard"><div class="nexusLoginHead"><div><span class="eyebrow">Patient account</span><h2>Sign Up</h2><p style="margin:0;color:#667085">Create an account to see transportation connected to your email.</p></div><button class="nexusLoginClose" type="button" aria-label="Close sign up">×</button></div><form class="nexusLoginForm"><label>Full name<input name="displayName" autocomplete="name" required></label><label>Email<input name="email" type="email" autocomplete="email" required></label><label>Password<input name="password" type="password" autocomplete="new-password" minlength="12" required><small>Use at least 12 characters.</small></label><label style="display:flex;grid-template-columns:auto 1fr;align-items:start"><input name="acceptTerms" type="checkbox" style="min-height:20px;width:20px" required><span>I agree to the <a href="/legal.html#terms">Terms</a> and acknowledge the <a href="/legal.html#privacy">Privacy Notice</a>.</span></label><button type="submit">Create patient account</button><p class="nexusLoginError" role="alert" aria-live="polite"></p></form></div>`;document.body.appendChild(dialog);dialog.querySelector('.nexusLoginClose').addEventListener('click',()=>dialog.close());dialog.addEventListener('click',event=>{if(event.target===dialog)dialog.close()});dialog.querySelector('form').addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,error=dialog.querySelector('.nexusLoginError'),button=form.querySelector('[type=submit]');error.textContent='';button.disabled=true;button.textContent='Creating account…';try{const response=await fetch('/api/auth/register',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({displayName:form.displayName.value.trim(),email:form.email.value.trim(),password:form.password.value,acceptTerms:form.acceptTerms.checked})});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||'Unable to create account');sessionStorage.setItem('nexusAccessToken',data.token);sessionStorage.setItem('nexusUser',JSON.stringify(data.user));location.assign('/livecare.html')}catch(caught){error.textContent=caught.message}finally{button.disabled=false;button.textContent='Create patient account'}});return dialog}
 function normalizeNav(){
  const header=document.querySelector('header');if(!header)return false;const nav=header.querySelector('#primary-navigation,.globalLinks,.links');if(!nav)return false;
  if(!nav.querySelector('.nexusHomeGroup')){nav.innerHTML='';const group=document.createElement('details');group.className='nexusHomeGroup';group.innerHTML=`<summary>Home</summary><div class="nexusHomeMenu">${sectionLinks.map(([label,id])=>`<a href="/#${id}">${label}</a>`).join('')}</div>`;nav.appendChild(group);const live=document.createElement('a');live.href='/livecare.html';live.textContent='LiveCare';nav.appendChild(live)}
